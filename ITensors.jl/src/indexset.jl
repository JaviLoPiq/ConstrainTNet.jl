...
#
# gives a tuple of integers for each index. Any pair of matching 
...
function flux(inds::Indices, block::Block) :: Union{Nothing, QNRegion, Vector{QRegion}}
  ...
  # Determine the type of each index
  ...
  # Process based on the number and types of indices
  if length(indx_qregions) == 1 && length(inds) == 2 # boundary term
      ind1, ind2 = indx_qregions[1], indx_qns[1]
      fl1, fl2 = flux(inds[ind1], Block(block[ind1])), flux(inds[ind2], Block(block[ind2]))
      
      if Int(dir(inds[ind1])) == -1 && Int(dir(inds[ind2])) == -1 # right boundary; assuming ortho center here
          return fl1 - fl2
      elseif Int(dir(inds[ind1])) == 1 && Int(dir(inds[ind2])) == -1 # left boundary; no ortho center here
          return (-fl2 ⊂ fl1) ? QN() : nothing
      end
  elseif length(indx_qregions) == 2 && length(inds) == 2
      fl1, fl2 = flux(inds[1], Block(block[1])), flux(inds[2], Block(block[2]))
      
      if typeof(fl1) <: QRegion && typeof(fl2) <: QRegion
          return fl1 + fl2
      elseif is_intersect(fl1, fl2)
          return QN(0)
      else
          return nothing
      end
  elseif length(indx_qregions) == 2 && length(inds) == 3
      ind1, ind2, ind3 = indx_qregions[1], indx_qregions[2], indx_qns[1]
      fl1, fl2, fl3 = flux(inds[ind1], Block(block[ind1])), flux(inds[ind2], Block(block[ind2])), flux(inds[ind3], Block(block[ind3]))
      
      if Int(dir(inds[ind1])) == -1 && Int(dir(inds[ind3])) == -1 && Int(dir(inds[ind2])) == 1
          return is_intersect(fl1 - fl3, fl2) ? QN(0) : nothing
      elseif Int(dir(inds[ind1])) == 1 && Int(dir(inds[ind3])) == -1 && Int(dir(inds[ind2])) == -1
          return is_intersect(fl2 - fl3, fl1) ? QN(0) : nothing
      elseif Int(dir(inds[ind1])) == -1 && Int(dir(inds[ind3])) == -1 && Int(dir(inds[ind2])) == -1
          return fl1 + fl2 - fl3
      else
          return nothing
  ...
  for n in 1:length(inds)
      qntot += flux(inds[n], Block(block[n]))
  end
