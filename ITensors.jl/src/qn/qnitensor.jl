...
"""
    ITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]inds)
    ITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]inds::Index...)
...
```
"""
function ITensor(::Type{ElT}, flux::QNRegion, inds::Indices) where {ElT<:Number}
  is = Tuple(inds)
...
function ITensor(::Type{ElT}, flux::QNRegion, is...) where {ElT<:Number}
  return ITensor(ElT, flux, indices(is...))
end

ITensor(flux::QNRegion, is...) = ITensor(Float64, flux, is...)
...
"""
    ITensor([::Type{ElT} = Float64,] ::UndefInitializer, flux::QNRegion, inds)
    ITensor([::Type{ElT} = Float64,] ::UndefInitializer, flux::QNRegion, inds::Index...)
...
the provided QNRegion `flux`. One purpose for using this constructor is that
...
"""
function ITensor(
  ::Type{ElT}, ::UndefInitializer, flux::QNRegion, inds::Indices
) where {ElT<:Number}
  is = Tuple(inds)
  blocks = nzblocks(flux, is)
...
function ITensor(::Type{ElT}, ::UndefInitializer, flux::QNRegion, is...) where {ElT<:Number}
  return ITensor(ElT, undef, flux, indices(is...))
end

function ITensor(::UndefInitializer, flux::QNRegion, is...)
  return ITensor(Float64, undef, flux, indices(is...))
end

"""
    ITensor([ElT::Type, ]x::Number, flux::QNRegion, inds)
    ITensor([ElT::Type, ]x::Number, flux::QNRegion, inds::Index...)

Construct an ITensor with all elements consistent with QNRegion flux `flux` set to `x` and indices `inds`.
...
function ITensor(eltype::Type{<:Number}, x::Number, flux::QNRegion, is...)
  return ITensor(eltype, x, flux, indices(is...))
end

ITensor(x::Number, flux::QNRegion, is...) = ITensor(eltype(x), x, flux, is...)

ITensor(x::RealOrComplex{Int}, flux::QNRegion, is...) = ITensor(float(x), flux, is...)

#Note: this should work for both QN and QNRegion 
...
of QNRegion indices. Zeros are dropped and nonzero blocks are determined
...
function emptyITensor(eltype::Type{<:Number}, flux::QNRegion, is...)
  return error(
    "Trying to create an empty ITensor with flux $flux, cannot create empty ITensor with a specified flux.",
  )
end
emptyITensor(flux::QNRegion, is...) = emptyITensor(EmptyNumber, flux, is...)

"""
    randomITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]inds)
    randomITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]inds::Index...)
...
"""
function randomITensor(::Type{ElT}, flux::QNRegion, inds::Indices) where {ElT<:Number}
  return randomITensor(Random.default_rng(), ElT, flux, inds)
end

function randomITensor(
  rng::AbstractRNG, ::Type{ElT}, flux::QNRegion, inds::Indices
) where {ElT<:Number}
...
function randomITensor(::Type{ElT}, flux::QNRegion, is...) where {ElT<:Number}
  return randomITensor(Random.default_rng(), ElT, flux, is...)
end

function randomITensor(rng::AbstractRNG, ::Type{ElT}, flux::QNRegion, is...) where {ElT<:Number}
...
function randomITensor(flux::QNRegion, inds::Indices)
...
function randomITensor(flux::QNRegion, is...)
  return randomITensor(Random.default_rng(), flux, is...)
end

function randomITensor(rng::AbstractRNG, flux::QNRegion, is...)
...
"""
    diagITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]is)
    diagITensor([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]is::Index...)
...
"""
function diagITensor(::Type{ElT}, flux::QNRegion, inds::Indices) where {ElT<:Number}
  is = Tuple(inds)
...
function diagITensor(::Type{ElT}, flux::QNRegion, is...) where {ElT<:Number}
  return diagITensor(ElT, flux, indices(is...))
end

function diagITensor(x::ElT, flux::QNRegion, inds::QNIndices) where {ElT<:Number}
  is = Tuple(inds)
  blocks = nzdiagblocks(flux, is)
...
function diagITensor(x::Number, flux::QNRegion, is...)
...
diagITensor(flux::QNRegion, is::Indices) = diagITensor(Float64, flux, is)

diagITensor(flux::QNRegion, is...) = diagITensor(Float64, flux, indices(is...))
...
"""
    delta([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]is)
    delta([::Type{ElT} = Float64, ][flux::QNRegion = QN(), ]is::Index...)
...
"""
function delta(::Type{ElT}, flux::QNRegion, inds::Indices) where {ElT<:Number}
  is = Tuple(inds)
  blocks = nzdiagblocks(flux, is)
...
function delta(::Type{ElT}, flux::QNRegion, is...) where {ElT<:Number}
  return delta(ElT, flux, indices(is...))
end

delta(flux::QNRegion, inds::Indices) = delta(Float64, flux, is)

delta(flux::QNRegion, is...) = delta(Float64, flux, indices(is...))
