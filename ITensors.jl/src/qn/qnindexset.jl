...
# TODO : can we use hashing or ordering for better comparison? eachblock checks all possible combinations
function nzblocks(qregion::QRegion, inds::Indices)
  N = length(inds)
  blocks = Block{N}[]
  for block in eachblock(inds)
    if is_intersect(flux(inds,block), qregion)
      push!(blocks, block)
    end
  end
  return blocks 
