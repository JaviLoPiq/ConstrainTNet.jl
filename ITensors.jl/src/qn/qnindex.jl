const QNBlock{T<:QNRegion} = Pair{T,Int64}

const QNBlocks{T<:QNBlock} = Vector{T}
...
#NDTensors.resize(qnblock::QNBlock{T<:QNRegion}, newdim::Int64) = QNBlock{T}(qnblock.first, newdim) 
NDTensors.resize(qnblock::QNBlock, newdim::Int64) = QNBlock{QNRegion}(qnblock.first, newdim) 
...
function -(qnb::QNBlock)
  return QNBlock{QNRegion}(-qn(qnb), blockdim(qnb))
end

function (qn1::QNBlock + qn2::QNBlock)
  qn(qn1) != qn(qn2) && error("Cannot add qn blocks with different qns")
  return QNBlock{QNRegion}(qn(qn1), blockdim(qn1) + blockdim(qn2))
end
...
function removeqn(space::QNBlocks, qn_name::String; mergeblocks=true)
  space = QNBlocks{QNBlock}([removeqn(qn_block, qn_name) for qn_block in space])
  if mergeblocks
...
"""
const QNIndex{T<:QNBlocks} = Index{T} 
...
#TODO: is this syntax correct? Wouldn't QNBlock{<:QNRegion} also work within function argument? 
# what about when creating new QNBlock, why do I need to specify the parameter type QNRegion, would also QNBlock{<:QNRegion} work?
# similarly for all functions involving QNBlock (check others are consistent)
(dir::Arrow * qnb::QNBlock) = QNBlock{QNRegion}(dir * qn(qnb), blockdim(qnb)) 
...
(qn1::QNBlock * qn2::QNBlock) = QNBlock{QNRegion}(qn(qn1) + qn(qn2), blockdim(qn1) * blockdim(qn2))

# TODO: rename tensorproduct with ⊗ alias
function outer(qn1::QNBlocks, qn2::QNBlocks)
  qnR = ITensors.QNBlocks{QNBlock}(undef, nblocks(qn1) * nblocks(qn2))
...
function HDF5.read(
  ...
  N = read(g, "length")
  dims = read(g, "dims")
  B = QNBlocks{QNBlock}(undef, N)
  for n in 1:length(B)
    B[n] = QNBlock{QNBlock}(read(g, "QN[$n]", QN), dims[n])
  end
  return B
end
